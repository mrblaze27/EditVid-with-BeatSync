...
def extract_clip_segment_ffmpeg(video_file: str, start_time: float, duration: float,
    ...
    try:
        ...
        # Scale to target size (ensuring even dimensions for H.264/NVENC compatibility)
        if target_size:
        ...
        base_cmd = [
            ...
            '-vf', filter_complex,
            '-vframes', str(output_frame_count)
        ]
        ...
        # First attempt: Try NVENC if requested
        if use_nvenc:
            cmd_nvenc = base_cmd + get_nvenc_quality_args(gpu_encoder, include_pix_fmt=True) + tail_cmd
            result = _run_media_command(cmd_nvenc, timeout=120)
            if result.returncode == 0 and os.path.exists(output_file) and os.path.getsize(output_file) > 0:
                return True, ""

        # Fallback (or CPU mode): Try CPU libx264
        cmd_cpu = base_cmd + get_cpu_h264_quality_args(include_pix_fmt=True) + tail_cmd
        result = _run_media_command(cmd_cpu, timeout=120)
        
        if result.returncode != 0:
            err_detail = _short_ffmpeg_error(result.stderr, 500)
            msg = f"FFmpeg error (code {result.returncode}): {err_detail}"
            try:
                sys.__stderr__.write(f"   ⚠️  {msg}\n")
        ...
        if not os.path.exists(output_file) or os.path.getsize(output_file) == 0:
            msg = f"FFmpeg output file missing or zero bytes: {output_file}"
            try:
                sys.__stderr__.write(f"   ⚠️  {msg}\n")
                sys.__stderr__.flush()
            except Exception:
        ...
        msg = f"Error extracting clip: {str(e)}"
        try:
            sys.__stderr__.write(f"   ⚠️  {msg}\n")
            sys.__stderr__.flush()
        except Exception:
