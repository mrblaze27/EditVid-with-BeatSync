...
def _effective_clip_workers(requested_workers: int, use_nvenc: bool) -> int:
    ...
    default_nvenc_workers = min(requested_workers, 2)
    return _env_int(
...
def create_clip_parallel(args):
    ...
    try:
        ...
        res = extract_clip_segment_ffmpeg(**extract_kwargs)
        if isinstance(res, tuple):
            success, err_msg = res
        else:
            success, err_msg = res, "FFmpeg extraction failed"
        
        elapsed = time.perf_counter() - clip_started
        if not success:
            return (i, None, target_size, None, err_msg or "FFmpeg extraction failed", elapsed)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    ...
                    if error:
                        err_msg = f"⚠️  Warning: Clip {i+1} failed after {_fmt_seconds(clip_elapsed)}: {error}"
                        print(err_msg)
                        try:
                            sys.__stdout__.write(err_msg + "\n")
